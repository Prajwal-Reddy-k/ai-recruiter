...
A targeted set of composite indexes was added for the hottest read paths (job search, applicant lists, candidate search's company-scoping join, message threads, notifications, interviews, audit-log lookups), based on the actual `.Where`/`.OrderBy` pairs used by each service — no speculative indexes for queries that don't exist, and no existing index (including the correctness-critical unique indexes on `Users.Email` and `JobApplications(JobPostingId, CandidateProfileId)`) touched.

- **Indexes added** (migration `AddPerformanceIndexes`, purely additive `CreateIndex` statements): `JobPostings(Status, ModerationStatus, CreatedAt)`, `JobPostings(CompanyId, Status, ModerationStatus, CreatedAt)`, `JobPostings(RecruiterProfileId, CreatedAt)`, `JobApplications(JobPostingId, CreatedAt)`, `JobApplications(CandidateProfileId, CreatedAt)`, `Messages(JobApplicationId, CreatedAt)`, `Notifications(UserId, CreatedAt)`, `Notifications(UserId, IsRead)`, `Interviews(JobApplicationId, CreatedAt)`, `Interviews(Status, ScheduledStartUtc)`, `AuditLogEntries(ActorUserId, TimestampUtc)`.
- **Not indexed on purpose**: free-text `Contains()` search on job title/skills/city/state (a leading-wildcard `LIKE '%x%'` can't seek a B-tree index — real full-text search is out of scope for a zero-budget project) and `CandidateSearchService`'s skills/sort filtering, which happens entirely in-memory after the SQL query returns and never reaches the database as a predicate.
- **Verification**: at the seeded dataset's current size (a few dozen rows per table), `SET STATISTICS IO` shows single-digit logical reads either way — too small a dataset to show a measurable before/after delta. The indexes exist for when the data grows past what fits in a few pages; verify their use on a larger dataset via SSMS/Azure Data Studio's execution plan (look for an Index Seek on the new composite index instead of a Table/Clustered Index Scan) or by seeding a much larger synthetic dataset.

**Manual test steps**: `dotnet ef database update`, then run any of the queries above via `sqlcmd`/SSMS with `SET STATISTICS IO, TIME ON` and confirm the query plan uses the new index (not a full scan) once the table has enough rows to matter.

## Environment-Specific Configuration Validation

Startup-time validation (`IValidateOptions<T>` + `.ValidateOnStart()`) now fails fast with a specific, actionable error if a **required-in-Production** setting is missing or invalid, while every **optional** integration keeps degrading gracefully exactly as before.

- **Required in Production**: `Jwt:Secret` (must be ≥32 characters — enforced in *every* environment, not just Production, since it's a security floor, not a deployment nicety) and `Jwt:Issuer`/`Jwt:Audience` (Production only); `ConnectionStrings:DefaultConnection` (Production only, skipped when the `UseInMemoryDatabase` test switch is set). Error messages name the exact missing/invalid key (e.g. `"Jwt:Secret is missing or shorter than 32 characters."`) — never the secret's actual value.
- **Optional, gracefully-degrading** (unchanged, no validator added): `Smtp`, `Cloudinary`, `Adzuna`, `Nominatim`, `ResumeStorage`, `Avatar`, `RateLimiting`, `Redis` — each already has an `IsConfigured`-gated fallback in `DependencyInjection.cs` (e.g. Smtp absent → `DevEmailSender` in Development, `NullEmailSender` in Production) that this feature deliberately leaves untouched.
- **What's relaxed in Development**: only the connection-string-required check (since `dotnet test`/local dev commonly runs against LocalDB or the in-memory provider) — the Jwt secret-length floor is *not* relaxed anywhere, and the shipped `appsettings.json` placeholder secret already satisfies it.

**Manual test steps**: run the API with `ASPNETCORE_ENVIRONMENT=Production` and an empty/short `Jwt:Secret` — confirm it throws `OptionsValidationException` naming `Jwt:Secret` before the app starts listening; restore a valid secret and confirm it starts normally.

## Containerization (Docker + docker-compose)

The full stack (API, frontend, SQL Server, plus Jaeger for tracing — see Observability below) now runs via `docker compose up --build` from a clean checkout, alongside the existing `dotnet run`/`npm run dev` workflow, which is completely unaffected and still the default for day-to-day development.

- **Setup**: copy `.env.example` to `.env` and fill in `SA_PASSWORD`/`JWT_SECRET` (never committed — `.env` is gitignored), then:
  ```bash
  docker compose up --build
  ```
  API on `http://localhost:5087`, frontend on `http://localhost:8080`, Jaeger UI on `http://localhost:16686`.
- **Migrations**: the `api` container runs with `AutoMigrate=true` (an env var set *only* in `docker-compose.yml`) so `Program.cs` applies pending EF Core migrations on startup automatically. The non-Docker path never sets this variable — local dev still runs `dotnet ef database update` manually, exactly as before.
- **Seeding**: the `api` container runs with `ASPNETCORE_ENVIRONMENT=Development` (a portfolio-demo choice, not a production pattern) so the existing demo-account seeder runs automatically, matching local dev.
- **Secrets**: the JWT secret and SQL Server password are supplied only via `docker-compose.yml`'s `environment:` block, sourced from your local `.env` — never baked into a Dockerfile `ENV`/`ARG` default or committed to the image.
- **Verified live**: a full `docker compose up --build` was run end-to-end in this environment — health check, login with a seeded demo account (`recruiter1@demo.airecruiter.dev` / `Demo@123`), and `GET /api/jobs` all returned success through the containerized stack, then torn down cleanly with `docker compose down`.

**Manual test steps**: `docker compose up --build`, open `http://localhost:8080`, log in with any seeded demo account (see `docs/SETUP_AND_DEMO.md`), browse jobs. Confirm `dotnet test` still passes unmodified outside Docker.

## Structured Logging and Observability

Serilog replaces the default logging provider (console + a daily-rolling file sink under `Logs/`) without changing any existing `_logger.LogInformation`/`LogWarning`/`LogError` call site's behavior, and OpenTelemetry adds distributed tracing + request metrics exported to a local Jaeger instance.

- **Structured request logging**: every request gets one log line (method, path, status code, duration) via Serilog's built-in `UseSerilogRequestLogging()`, enriched with the authenticated user id (never the JWT itself) when the caller is signed in.
- **Log level per environment**: purely config-driven via the `Serilog` section in `appsettings.json` (`Warning` default) and `appsettings.Development.json` (`Information` default) — no code change needed to adjust it.
- **Tracing**: `OpenTelemetry.Instrumentation.AspNetCore`/`.Http`/`.EntityFrameworkCore` export spans via OTLP to `OpenTelemetry:OtlpEndpoint` (default `http://localhost:4317`) — in Docker Compose this points at the bundled `jaeger` service; standalone, run `docker run -d -p 16686:16686 -p 4317:4317 jaegertracing/all-in-one:latest` and view traces at `http://localhost:16686`.
- **Sensitive-data audit**: every existing `ILogger` call site (8 files) was reviewed — none log a password, JWT, resume content, or raw PII beyond what's already elsewhere in the app; the one intentional exception (`DevEmailSender` logging email body content) is pre-existing, Development-only-by-construction (it exists specifically so a developer can see an email that would otherwise go nowhere since no real SMTP is configured), and unrelated to this feature.

**Manual test steps**: hit any endpoint (e.g. `GET /health`) and confirm a structured line appears in `Logs/log-<date>.txt` and the console; with Jaeger running, make a few requests (login, job search) and confirm traces appear at `http://localhost:16686`.

## Redis-Backed Distributed Cache

The Adzuna external-search response cache and the anonymous job-view-count dedup mechanism can now optionally run on Redis instead of the existing in-memory implementations — selected automatically at startup based on whether `Redis:ConnectionString` is configured, with **identical cache semantics** (same TTLs, same key formats) either way, so switching providers never changes observable app behavior, only scalability.

- **Default (no Redis configured)**: exactly today's behavior — `InMemoryViewDeduplicationService` (30-minute sliding window) and the Adzuna service's original in-process `IMemoryCache` (24-hour TTL), now behind a small `IExternalJobSearchCache` seam (`MemoryExternalJobSearchCache`) but functionally unchanged.
- **Redis configured**: `RedisViewDeduplicationService` (atomic `SET NX` + expiry, same key format `"{jobId}:{visitorKey}"`, same 30-minute window) and `RedisExternalJobSearchCache` (JSON-serialized results, same key format, same 24-hour TTL) are used instead.
- **Graceful degradation**: a Redis connection failure is caught and logged as a warning, then treated as a cache miss / "should count this view" — never an unhandled exception or a failed request.
- **Local setup**: `docker run -d -p 6379:6379 redis:7-alpine`, then set `Redis__ConnectionString=localhost:6379` — or in Docker Compose, the bundled `redis` service is wired automatically. A small managed Redis free tier (e.g. Upstash/Redis Cloud's free plan) is sufficient for light production use beyond local dev.

**Manual test steps**: with no `Redis:ConnectionString` set, confirm job search and view-counting work exactly as before; set it to a local Redis, restart the API, repeat the same actions, and confirm identical observable behavior (only the storage backend changed).
