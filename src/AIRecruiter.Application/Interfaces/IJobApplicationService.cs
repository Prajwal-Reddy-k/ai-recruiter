...
{
    Task<JobApplicationDto> ApplyAsync(int candidateUserId, int jobPostingId, string? coverNote, IReadOnlyList<SubmitScreeningAnswerRequest>? answers = null, CancellationToken ct = default);
    ...
    Task<IReadOnlyList<JobApplicationDto>> GetApplicationsForJobAsync(int recruiterUserId, int jobPostingId, ApplicantScreeningFilterQuery? filter = null, CancellationToken ct = default);
